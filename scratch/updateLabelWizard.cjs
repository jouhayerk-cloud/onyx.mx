...
let content = fs.readFileSync('src/features/logistics/LabelWizard.tsx', 'utf-8');

// 1. Add CircularProgress component before LabelWizard
const circularProgressCode = `import { createPortal } from 'react-dom';
...
content = content.replace('export const LabelWizard: React.FC = () => {', circularProgressCode);

// 2. Add isNameEditing state
const stateCode = `
...
// 3. Replace the return statement of LabelWizard
const returnStart = `    if (!isOpen) return null;
...
const returnEnd = `                <div className="mt-auto pt-16 flex justify-between items-end opacity-5"><Cpu size={16} strokeWidth={1} /></div>
            </div>

            {/* ── LABEL PREVIEW OVERLAY — Fullscreen Glass Panel ── */}`;

const blockToReplace = content.substring(content.indexOf(returnStart), content.indexOf(returnEnd) + returnEnd.length);

content = content.replace(blockToReplace, newReturn + `\n\n            {/* ── LABEL PREVIEW OVERLAY — Fullscreen Glass Panel ── */}`);

// We also need to add document.body to the end of createPortal
const endDiv = `        </div>
    );
};`;
...
console.log('Successfully updated LabelWizard.tsx');
