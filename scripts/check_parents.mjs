...
async function checkCrates() {
    const { data: crates } = await supabase.from('logistics').select('id, parent_id').in('id', ids);
    
    let parentCount = 0;
    crates.forEach(c => {
        if (c.parent_id) {
            parentCount++;
            console.log(`Crate ${c.id.slice(0,8)} has parent!`);
        }
    });
    console.log(`Out of ${crates.length} crates, ${crates.length - parentCount} have no parent.`);
}
checkCrates();
