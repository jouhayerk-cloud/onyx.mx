...
async function debug() {
    const { data: crates, error } = await supabase.from('logistics').select('id, description, label, status').eq('status', 'Deployed');
    if (error) {
        console.error("Supabase Error:", error);
        return;
    }
    console.log(`Successfully fetched ${crates.length} deployed crates.`);
}
debug();
