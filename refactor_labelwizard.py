...
with open(r'c:\Jouhayerk\git\app\src\features\logistics\LabelWizard.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
# 2. Replace the JSX using regex
pattern = re.compile(
    r'<div className="flex justify-between items-start mb-16 shrink-0">.*?<div className="mt-auto pt-16 flex justify-between items-end opacity-5"><Cpu size={16} strokeWidth={1} /></div>\s*</div>',
    re.DOTALL
)

new_jsx = """<div className="flex justify-between items-start mb-8 shrink-0">
...
if not pattern.search(content):
    print("Could not find the target JSX block!")
    exit(1)

content = pattern.sub(new_jsx, content)

with open(r'c:\Jouhayerk\git\app\src\features\logistics\LabelWizard.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Successfully refactored using Python.")
