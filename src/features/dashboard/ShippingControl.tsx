...
import {
    ...
    truckViewSelectedWarehouseCrateIdAtom,
    tempCratePositionAtom,
...
const WarehouseViewControls = ({ saveCratesToBackend }: { saveCratesToBackend: (crates: Crate[], message: string) => Promise<void> }) => {
    const [crates, setCrates] = useAtom(shippingCratesAtom);
    const inventory = useAtomValue(inventoryAtom);
    const [selectedCrateId, setSelectedCrateId] = useAtom(selectedCrateIdAtom);
    ...
    const handleUnloadItem = async (itemToUnload: PackedItem) => {
        ...
        try {
            const crateUpdatePromise = fetch(SCRIPT_URL, {
            ...
            const [crateRes, itemRes] = await Promise.all([crateUpdatePromise, itemUpdatePromise]);
            const crateResult = await crateRes.json();
            const itemResult = await itemRes.json();

            if (crateResult.status !== 'success' || itemResult.status !== 'success') {
                throw new Error(`Crate: ${crateResult.message}, Item: ${itemResult.message}`);
            }
    ...
    const uniqueVendors = ['All', ...new Set(itemsToShip.map(i => i.data.itemId))];
    return (
        <>
            <div className="shipping-sidebar-section">
                <h2>{tr("Ready to Ship")}</h2>
...
const TruckViewControls = ({ saveCratesToBackend }: { saveCratesToBackend: (crates: Crate[], message: string) => Promise<void> }) => {
    ...
    const [selectedCrateId, setSelectedCrateId] = useAtom(selectedCrateIdAtom);
    const [truckCrateId, setTruckCrateId] = useAtom(truckViewSelectedWarehouseCrateIdAtom);
    const [tempPosition, setTempPosition] = useAtom(tempCratePositionAtom);
...
export const ShippingControl = ({ isVisible }: { isVisible: boolean }) => {
    const viewMode = useAtomValue(shippingViewModeAtom);
    const [crates, setCrates] = useAtom(shippingCratesAtom);
    const [isSaving, setIsSaving] = useState(false);
    const [areLabelsVisible, setAreLabelsVisible] = useAtom(areCrateInfoLabelsVisibleAtom);
    const user = useAtomValue(userAtom);
    const setCratesVersion = useSetAtom(cratesVersionAtom);
    const triggerOrganization = useAtomValue(triggerWarehouseOrganizationAtom);
    const warehouseDims = useAtomValue(WAREHOUSE_DIMS);
