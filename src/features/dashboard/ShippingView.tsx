...
import {
    ...
    isShippingSidebarCollapsedAtom,
...
export function ShippingView({ mode = 'live' }: { mode?: 'live' | 'archive' }) {
    ...
    const setCrates = useSetAtom(shippingCratesAtom);
        ...
        <div className={`shipping-view-container ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
            ...
            <main className="shipping-main">
                ...
                <div ref={canvasContainerRef} id="canvasContainer" className="w-full h-full">
                    <ThreeScene setCrates={setCrates} cratesVersion={cratesVersion} user={user} store={store} />
                </div>
            </main>
        </div>
    );
...
const ThreeScene = ({ setCrates, cratesVersion, user, store }: { setCrates: any, cratesVersion: number, user: any, store: any }) => {
    ...
    useEffect(() => {
        ...
        const updateCamera = () => {
            const view = store.get(shippingCameraViewAtom);
            const viewMode = store.get(shippingViewModeAtom);
            const selectedId = store.get(selectedCrateIdAtom);
            const crates = store.get(shippingCratesAtom);
