      ...
      <div className="w-full max-w-lg bg-neutral-900 border border-purple-500/30 rounded-2xl p-6 shadow-[0_0_50px_rgba(168,85,247,0.2)] relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-white tracking-wide">{tr("Web Bluetooth (BLE) Pairing")}</h3>
              ...
              <p className="text-[11px] font-mono text-neutral-400">Discover and pair directly with <strong className="text-purple-300">{tr("OnyxChan-XXXX")}</strong></p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {!isWebBluetoothSupported && (
          <div className="mb-5 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-start gap-3 text-amber-300 text-xs">
            ...
            <div>
              <strong className="font-bold block mb-1">{tr("Web Bluetooth Not Detected")}</strong>
              Your browser does not support the Web Bluetooth API. Please launch Onyx.mx in <strong>{tr("Google Chrome")}</strong> or <strong>{tr("Microsoft Edge")}</strong> {tr("with Bluetooth enabled on your computer or mobile device.")}
            </div>
          </div>
        )}
        ...
        <div className="p-5 rounded-2xl bg-black/50 border border-white/10 mb-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
          {state.status === 'connected' ? (
            <>
              ...
              <h4 className="text-sm font-bold text-white mb-1">{tr("Paired & Connected!")}</h4>
              ...
              <div className="grid grid-cols-3 gap-3 w-full max-w-xs p-3 rounded-xl bg-white/5 border border-white/5 text-center">
                <div>
                  <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 block mb-0.5">{tr("Battery")}</span>
                ...
                <div>
                  <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 block mb-0.5">{tr("Channel")}</span>
                  <span className="text-xs font-mono font-bold text-purple-300">{tr("BLE UART")}</span>
                </div>
                <div>
                  <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 block mb-0.5">{tr("Status")}</span>
                  <span className="text-xs font-mono font-bold text-cyan-400">{tr("Active")}</span>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <h4 className="text-sm font-bold text-white mb-1">{tr("Searching for OnyxChan BLE Beacons...")}</h4>
              <p className="text-xs font-mono text-neutral-400">
                {tr("Select your")} <strong>{tr("OnyxChan-XXXX")}</strong> or M5Stack CoreS3 in the browser prompt.
              </p>
            </>
          ) : (
            <>
              ...
              <h4 className="text-sm font-bold text-white mb-1">{tr("Ready to Pair Robot")}</h4>
              <p className="text-xs font-mono text-neutral-400 max-w-sm">
                {tr("Ensure your StackChan desktop robot is powered on with BLE firmware active.")}
              </p>
            </>
          )}
        ...
        <div className="space-y-2 mb-6 text-xs text-neutral-400 bg-white/[0.02] p-4 rounded-xl border border-white/5 font-mono">
          ...
          <div className="flex items-start gap-2">
            ...
            <span>{tr("Power on your M5Stack CoreS3 / StackChan robot.")}</span>
          </div>
          <div className="flex items-start gap-2">
            ...
            <span>{tr("Click")} <strong>{tr("\"Start BLE Discovery\"")}</strong> {tr("below to open the native browser BLE picker.")}</span>
          </div>
          <div className="flex items-start gap-2">
            ...
            <span>{tr("Select")} <strong>{tr("OnyxChan-XXXX")}</strong> from the device list to establish zero-latency control.</span>
          </div>
        </div>
