            ...
            <header className="border-b border-gray-200 sticky top-0 bg-white/95 backdrop-blur-md z-40 shadow-sm">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center space-x-6 text-gray-700">
                        <button className="hover:text-black transition-colors flex items-center gap-1.5" title={tr("User Profile")}>
                            <User className="w-6 h-6 stroke-[1.5]" />
                        </button>
                        <button onClick={handleAddToCart} className="hover:text-black transition-colors relative flex items-center" title={tr("Shopping Cart")}>
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between whitespace-nowrap text-sm font-medium h-12">
                        ...
                        <div className="flex items-center space-x-6 pl-4 border-l border-gray-200 py-1">
                            ...
                            <a href="tel:4805754360" className="bg-gray-900 hover:bg-gray-800 text-white px-3.5 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-sm">
                                <Phone className="w-3 h-3 fill-current" /> {tr("Call Us")}
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white py-3 px-4 sm:px-6 lg:px-8 shadow-inner">
                <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4 text-xs sm:text-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-300 font-medium">{tr("Rare Earth Gallery Store UI Clone & AI Preview")}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {selectedItem && (
                            ...
                            >
                                <Grid className="w-3.5 h-3.5" /> {tr("Back to Catalog Grid")}
                            </button>
                        )}
                    </div>
                </div>
                ...
                {selectedItem && pdpData ? (
                    ...
                    <div className="animate-fadeIn">
                        ...
                        <nav className="flex items-center text-xs text-gray-500 mb-6 space-x-2">
                            <a href="#home" onClick={(e) => { e.preventDefault(); setSelectedItem(null); }} className="hover:text-black">{tr("Home")}</a>
                            <span>›</span>
                                ...
                                <div className="aspect-square w-full bg-black rounded-lg overflow-hidden relative flex items-center justify-center p-8 shadow-2xl border border-gray-800 group">
                                    ...
                                    {pdpData.norm.generatedPngUrl && (
                                        <div className="absolute top-4 left-4 bg-gray-900/90 text-amber-300 text-[11px] font-semibold px-3 py-1 rounded-full border border-gray-700 backdrop-blur-md flex items-center gap-1.5 shadow-lg">
                                            <Sparkles className="w-3.5 h-3.5 text-amber-400" /> {tr("WebP Background Removed")}
                                        </div>
                                    )}
                                </div>
                                    ...
                                    {activeTab === 'desc' ? (
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 space-y-4 text-sm">
                                            <div className="grid grid-cols-2 gap-4 border-b border-gray-200 pb-3">
                                                <span className="text-gray-500 font-medium flex items-center gap-2"><Ruler className="w-4 h-4 text-gray-400" /> {tr("Dimensions:")}</span>
                                                <span className="font-semibold text-gray-900">{pdpData.dimsImp} ({pdpData.wCm} x {pdpData.hCm} x {pdpData.dCm} {tr("cm)")}</span>
                                            </div>
                                            <div className="grid grid-cols-2 gap-4 border-b border-gray-200 pb-3">
                            ...
                            <div className="lg:col-span-5 flex flex-col">
                                <div className="sticky top-28 bg-white p-2">
                                    ...
                                    <p className="text-xs text-gray-600 underline cursor-pointer mb-2">
                                        {tr("Shipping calculated at checkout.")}
                                    </p>
                                    <p className="text-xs text-gray-600 mb-6 leading-relaxed">
                                        From <span className="font-bold text-gray-900">${Math.max(15, Math.round(pdpData.priceNum / 20))}/mo</span> or 0% APR with <span className="font-bold text-purple-700 bg-purple-100 px-1.5 py-0.5 rounded text-[11px]">{tr("shop pay")}</span>. <a href="#sample" onClick={(e) => { e.preventDefault(); toast(tr("0% APR financing available up to 24 mos.")); }} className="underline font-medium hover:text-black">{tr("View sample plans")}</a>
                                    </p>
                                    ...
                                    {/* Buy It Now Button */}
                                    <button
                                        onClick={() => toast.success(tr("Simulated Express Buy It Now!"), { icon: '⚡' })}
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-5 border border-gray-200 flex items-start gap-3 text-xs text-gray-700">
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <p className="font-semibold text-gray-900">
                                                Pickup available at <span className="font-bold">{tr("Rare Earth Gallery")}</span>
                                            </p>
                                    ...
                                    <div className="mt-8 pt-6 border-t border-gray-100 space-y-2 text-[11px] text-gray-500">
                                        <div className="flex items-center justify-between">
                                            <span className="flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> {tr("AI Metadata Verified:")}</span>
                                            <span className="font-mono text-gray-700 font-bold">{tr("100% Complete")}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="aspect-square w-full bg-black relative overflow-hidden flex items-center justify-center p-6 border-b border-gray-100">
                                                ...
                                                {isFullyGenerated(item) && (
                                                    <div className="absolute top-2.5 left-2.5 bg-gray-900/80 text-amber-300 text-[9px] font-semibold px-2 py-0.5 rounded-full backdrop-blur-sm border border-gray-700 flex items-center gap-1 shadow-sm">
                                                        <Sparkles className="w-2.5 h-2.5 text-amber-400" /> {tr("100% AI Ready")}
                                                    </div>
                                                )}
                                            </div>
            ...
            <footer className="bg-gray-900 text-white mt-auto py-12 px-4 sm:px-6 lg:px-8 border-t border-gray-800">
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 text-xs text-gray-400">
                    <div className="space-y-3">
                        <img src={RARE_EARTH_LOGO} alt={tr("REG Logo")} className="h-10 w-auto brightness-200 grayscale" onError={(e) => { (e.target as HTMLImageElement).src = '/REG_Logo.png'; }} />
                        <p className="leading-relaxed">
                            The 8th Wonder of the World. Offering museum-grade mineral specimens, gemstone jewelry, and custom stone furnishings.
                        </p>
                    </div>
                    <div>
                        <h4 className="text-white font-bold uppercase tracking-wider mb-3 text-sm">{tr("Gallery Location")}</h4>
                        <p>{tr("6401 E. Cave Creek Rd.")}</p>
                        <p>{tr("Cave Creek, AZ 85331")}</p>
                        <p className="mt-2">{tr("Phone: (480) 575-4360")}</p>
                    </div>
                    <div>
                        <h4 className="text-white font-bold uppercase tracking-wider mb-3 text-sm">{tr("Customer Care")}</h4>
                        <ul className="space-y-1.5">
                            <li><a href="#shipping" onClick={(e) => e.preventDefault()} className="hover:text-white transition-colors">{tr("Shipping & Delivery")}</a></li>
                            <li><a href="#returns" onClick={(e) => e.preventDefault()} className="hover:text-white transition-colors">{tr("Return Policy")}</a></li>
                            <li><a href="#faq" onClick={(e) => e.preventDefault()} className="hover:text-white transition-colors">{tr("Frequently Asked Questions")}</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4 className="text-white font-bold uppercase tracking-wider mb-3 text-sm">{tr("Onyx.mx AI Engine")}</h4>
                        <p className="leading-relaxed">
                            {tr("Powered by Google DeepMind Advanced Agentic Coding. Generating automated spatial masks, background removal, and marketing copy for natural stone artifacts.")}
                        </p>
                        <div className="mt-4 flex items-center gap-2 text-amber-400 font-mono text-[11px]">
                            <Sparkles className="w-4 h-4" /> {tr("v2.4-REG Production Preview")}
                        </div>
                    </div>
                </div>
            </footer>
