...
export function StoreView() {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden bg-(--background-color) relative">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 border-b border-white/5 gap-4">
                <div className="flex-1 w-full relative group">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                        <Search size={18} strokeWidth={2} className="text-white/40 group-focus-within:text-(--main-color) transition-colors" />
                    </div>
                    <input
                        type="text"
                        value={searchTerm}
                        ...
                        className="w-full max-w-xl bg-white/5 border border-white/10 rounded-2xl py-3 pl-11 pr-10 text-sm text-white outline-none placeholder-white/40 focus:bg-white/10 focus:border-white/20 transition-all shadow-lg backdrop-blur-md font-medium"
                    />
                    {searchTerm && (
                        <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-auto left-[calc(100%-40px)] sm:left-auto sm:right-0 sm:relative flex items-center sm:pr-4 text-white/30 hover:text-white/70 transition-colors ml-[-30px] sm:ml-0 z-10 w-8 justify-center">
                        ...
                        {filteredItems.map(item => {
                            ...
                            return (
                                <div
                                    ...
                                    className="bg-white/[0.03] border border-white/5 rounded-2xl overflow-hidden hover:border-(--main-color)/40 hover:bg-white/[0.05] transition-all duration-300 group flex flex-col cursor-pointer relative shadow-lg hover:shadow-2xl hover:shadow-(--main-color)/5 hover:-translate-y-1"
                                >
                                    ...
                                    <div className="p-4 flex flex-col gap-1.5 flex-1 relative z-10 bg-gradient-to-t from-black/40 to-transparent">
            ...
            {selectedItem && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl animate-in fade-in duration-300 p-4 sm:p-8">
                    <button onClick={closePanel} className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-all z-50 shadow-xl border border-white/10 hover:scale-110">
                        <X size={24} strokeWidth={2} />
                    </button>

                    <div className="w-full max-w-6xl h-full max-h-[90vh] bg-[#0f0f13] rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row border border-white/10 relative">

                        {/* Media Gallery Section */}
                        <div className="w-full md:w-3/5 lg:w-2/3 h-64 md:h-full bg-black relative flex items-center justify-center group overflow-hidden border-b md:border-b-0 md:border-r border-white/5">
                            {(() => {
                                const mediaFiles = (selectedItem.data as any)._allMedia || [];
                                if (mediaFiles.length === 0) {
                                    return <span className="text-white/30 font-bold uppercase tracking-widest">No Media Available</span>;
                                }
                                        ...
                                        {mediaFiles.length > 1 && (
                                            <>
                                                <button
                                                    ...
                                                    className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/80 text-white rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all hover:scale-110 border border-white/10"
                                                >
                                                    <ChevronLeft size={24} />
                                                </button>
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setGalleryIndex(prev => prev === mediaFiles.length - 1 ? 0 : prev + 1); }}
                                                    className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/80 text-white rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all hover:scale-110 border border-white/10"
                                                >
                                                    <ChevronRight size={24} />
                                                </button>

                                                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-2 bg-black/50 backdrop-blur-md rounded-2xl border border-white/10">
                                                    {mediaFiles.map((_: any, idx: number) => (
                                                        <button
                                                            key={idx}
                                                            onClick={() => setGalleryIndex(idx)}
                                                            className={`w-2 h-2 rounded-full transition-all ${idx === galleryIndex ? 'bg-(--main-color) w-6' : 'bg-white/30 hover:bg-white/60'}`}
                                                        />
                                                    ))}
                                                </div>
                                            </>
                        ...
                        {/* Details Section */}
                        <div className="w-full md:w-2/5 lg:w-1/3 h-full overflow-y-auto p-6 md:p-10 flex flex-col custom-scrollbar bg-gradient-to-br from-white/[0.02] to-transparent">
                            ...
                            <h2 className="text-3xl font-black text-white mb-2 leading-tight">
                                {selectedItem.data.shape} <span className="text-white/50">{selectedItem.data.material}</span>
                            </h2>
                            <p className="text-sm font-bold text-white/60 uppercase tracking-widest mb-6 pb-6 border-b border-white/10">
                                {selectedItem.data.color}
                            </p>

                            <div className="flex-1 space-y-6">
                                {selectedItem.data.short_description && (
                                    <div>
                                        <h3 className="text-[10px] text-white/40 uppercase tracking-widest font-black mb-2 opacity-70">About</h3>
                                ...
                                <div>
                                    <h3 className="text-[10px] text-white/40 uppercase tracking-widest font-black mb-3 opacity-70">Specifications</h3>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                                            <span className="block text-[9px] text-white/40 uppercase tracking-wider mb-1">Dimensions</span>
                                            <span className="text-xs font-mono font-bold text-white/90">
                                                {selectedItem.data.lengthCm}x{selectedItem.data.widthCm}x{selectedItem.data.heightCm} cm
                                            </span>
                                        </div>
                                        <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                                            <span className="block text-[9px] text-white/40 uppercase tracking-wider mb-1">Weight</span>
                                            <span className="text-xs font-mono font-bold text-white/90">
                                        ...
                                        {!isVendor && selectedItem.data.bookAqCode && (
                                            <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                                                <span className="block text-[9px] text-white/40 uppercase tracking-wider mb-1">AQC Code</span>
                                        ...
                                        {!isVendor && selectedItem.data.bookLanded && (
                                            <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                                                <span className="block text-[9px] text-white/40 uppercase tracking-wider mb-1">Landed</span>
                                                <span className="text-xs font-mono font-bold text-(--main-color)">
                                        ...
                                        {!isVendor && selectedItem.data.bookRetail && (
                                            <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                                                <span className="block text-[9px] text-white/40 uppercase tracking-wider mb-1">Retail</span>
                            ...
                            <div className="mt-8 pt-8 border-t border-white/10 flex items-end justify-between gap-4">
                                <div>
                                    <span className="block text-[10px] text-white/40 uppercase tracking-widest font-black mb-1">Acquisition Price</span>
            ...
            {isBagOpen && (
                <div className="absolute inset-y-0 right-0 w-full sm:w-[400px] bg-black/95 backdrop-blur-2xl border-l border-white/10 flex flex-col z-50 animate-in slide-in-from-right-8 duration-300 shadow-2xl">
                    <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
                        <div className="flex items-center gap-3">
                            <ShoppingBag className="w-5 h-5 text-(--main-color)" />
                            <h2 className="text-sm font-black uppercase tracking-widest text-white">Acquisition Bag</h2>
                        </div>
                        <button onClick={() => setIsBagOpen(false)} className="p-2 text-white/50 hover:text-white bg-white/5 hover:bg-white/10 rounded-full transition-all">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 custom-scrollbar">
                        {shoppingBag.length === 0 ? (
                            ...
                            shoppingBag.map(item => (
                                <div key={item.row} className="flex items-center gap-4 p-4 bg-white/[0.03] hover:bg-white/[0.08] rounded-2xl border border-white/5 transition-colors group">
                                    <div className="w-16 h-16 bg-black/40 rounded-xl flex items-center justify-center p-0.5 overflow-hidden shadow-inner border border-white/5 shrink-0">
                                        {item.imageUrl?.match(/\.(mp4|webm|ogg|mov)$/i) ? (
                                            ...
                                            <svg className="w-6 h-6 opacity-20 text-white"><use href="#image" /></svg>
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <p className="text-xs font-bold text-white truncate leading-tight mb-1">{item.data.shape} <span className="text-white/60">{item.data.material}</span></p>
                                        <p className="text-[10px] text-white/40 font-black uppercase tracking-widest truncate">{item.data.color}</p>
                                        <p className="text-xs text-(--main-color) font-black mt-2">{getPriceLabel(item.data)}</p>
                                    </div>
                                    <button
                                        onClick={() => handleRemoveFromCart(item.row)}
                                        className="p-2 text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded-xl transition-colors opacity-50 group-hover:opacity-100"
                                    >
                    ...
                    <div className="p-6 border-t border-white/10 bg-black/40 backdrop-blur-md flex flex-col gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
                        <div className="flex justify-between text-[11px] font-bold tracking-widest text-white/50 uppercase">
                        ...
                        {isVendor && (
                            <div className="flex justify-between text-[11px] font-bold tracking-widest text-white/50 uppercase">
                                <span>IVA (15%)</span>
                                <span className="text-white">${cartIva.toFixed(2)}</span>
                            </div>
                        )}
                        <div className="flex justify-between text-lg font-black tracking-widest text-white uppercase mt-2 pt-4 border-t border-white/10">
                            <span>Total</span>
                            <span className="text-(--main-color)">{isVendor ? 'MXN' : 'MXN'} ${finalTotal.toFixed(2)}</span>
                        </div>
                        <button
                            disabled={shoppingBag.length === 0}
                            onClick={handleCheckout}
                            className={`mt-4 w-full py-4 rounded-xl text-sm font-black tracking-widest transition-all ${shoppingBag.length > 0 ? 'bg-(--main-color) text-black hover:brightness-110 cursor-pointer shadow-lg shadow-(--main-color)/20 hover:-translate-y-1' : 'bg-white/5 border border-white/10 text-white/20 cursor-not-allowed'}`}
                        >
