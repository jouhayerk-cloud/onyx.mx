    ...
    if (!suppliesInfo || suppliesInfo.items.length === 0) {
        return (
            <div className="flex items-center justify-center h-full text-[var(--text-color-secondary)]">
                {tr("No supplies or labor data found in -Supplies sheet.")}
            </div>
        );
    }
        ...
        <div className="p-6 flex flex-col gap-6 h-full overflow-y-auto custom-scrollbar relative z-10">
            <div className="flex justify-between items-end border-b border-white/10 pb-4">
                <div className="flex flex-col">
                    <span className="text-[10px] uppercase tracking-[0.2em] text-white/40 font-bold">{tr("Inventory")}</span>
                    <h2 className="text-3xl font-black text-white italic font-display">{tr("SUPPLIES & LABOR")}</h2>
                </div>
                <div className="flex gap-6">
                    <div className="flex flex-col items-end">
                        <span className="text-[9px] uppercase tracking-widest text-white/30">{tr("Total Out")}</span>
                    ...
                    <div className="flex flex-col items-end">
                        <span className="text-[9px] uppercase tracking-widest text-white/30">{tr("Remaining")}</span>
                ...
                <table className="w-full text-left">
                    <thead className="bg-white/[0.02] border-b border-white/5">
                        <tr className="text-[10px] uppercase tracking-widest text-white/40 font-bold">
                            <th className="p-4">{tr("Date")}</th>
                            <th className="p-4">{tr("Item / Description")}</th>
                            <th className="p-4 text-center">{tr("Qty")}</th>
                            <th className="p-4 text-right">{tr("Amount (MXN)")}</th>
                            <th className="p-4 text-right">{tr("Total")}</th>
