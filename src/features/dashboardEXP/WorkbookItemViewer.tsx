        ...
        <div className="project-box-wrapper">
            <div className="project-box" style={{ gridTemplateColumns, gap: '1rem', alignItems: 'center' }} onClick={() => setExpanded(!expanded)}>
                ...
                <div className="box-content-line gap-1 overflow-hidden">
                    ...
                    <p className="text-xs font-semibold mt-1">{tr("Qty:")} {item['Q'] || '0'}</p>
                </div>

                {/* Codes and Barcode */}
                <div className="box-content-line gap-1 overflow-hidden">
                     <div className="codes-panel flex gap-4 text-xs">
                        <div className="flex items-baseline gap-1.5"><strong>{tr("AQC:")}</strong> <span>{calculated.bookAqCode || item['AQC'] || '-'}</span></div>
                        <div className="flex items-baseline gap-1.5"><strong>{tr("LC:")}</strong> <span>{calculated.bookLandCode || item['LC'] || '-'}</span></div>
                    </div>
                    <p className="font-mono text-base font-bold opacity-90 pt-1" title={tr("Barcode")}>{tr("SKU:")} {item['TAG-ID'] || '-'}</p>
                    <p className="font-mono text-xs opacity-70" title={tr("Generated Barcode")}>{calculated.bookBardcode || 'N/A'}</p>
                </div>

                {/* Dimensions Panel */}
                <div className="dimensions-panel">
                    <div className="grid grid-cols-[auto_1fr_1fr] gap-x-2 text-xs">
                        <strong></strong><span className="text-right opacity-70">{tr("Metric")}</span><span className="text-right opacity-70">{tr("Imperial")}</span>
                        ...
                        <strong>{tr("Wt:")}</strong> <span className="text-right">{weightKg > 0 ? `${weightKg.toFixed(1)}kg` : '-'}</span> <span className="text-right">{weightLbs > 0 ? `${weightLbs.toFixed(1)}lb`: '-'}</span>
                    </div>
                </div>
            </div>
